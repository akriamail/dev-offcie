...
- `ipset k3s_nodes` 当前配置为 `192.168.1.240/241/242`（按实际节点 IP 更新）。
- 放行规则使用 `192.168.1.0/24` 与 `39.107.113.26/32`。
- 未完成（仍待执行）:
...
- 重要: UDP/53 需走 gw 本地 DNS（见 4.2），否则会被 DNS 劫持导致镜像拉取失败。

## 4.2 DNS 透明代理（已落地）

- 背景: 直连 DNS 会被劫持，导致 `registry-1.docker.io` 解析到错误 IP，出现 TLS 失败。
- 方案: gw 本机提供 DNS 转发，节点 UDP/53 透明重定向到 gw。
- 组件:
- `dnscrypt-proxy` 监听 `127.0.0.1:5053`，通过 `socks5://127.0.0.1:17890` 走 HK 出口（DoH）。
...
- 持久化:
- `dnscrypt-proxy`/`dnsmasq`/`sing-box`/`netfilter-persistent` 需保持 `enabled`。
- `iptables` 规则保存在 `/etc/iptables/rules.v4`，`ipset` 保存在 `/etc/iptables/ipsets`。

### 4.2.1 验证命令（gw / 节点）

```bash
# gw
dig +short @127.0.0.1 -p 5053 registry-1.docker.io
dig +short @192.168.1.100 registry-1.docker.io

# 节点
getent ahosts registry-1.docker.io | head -n 5
...
EOF
sysctl --system | grep disable_ipv6

# 指向 gw 做 DNS
resolvectl dns ens160 192.168.1.100
